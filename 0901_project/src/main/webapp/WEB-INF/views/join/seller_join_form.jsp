...
<html>
	<head>
		...
		<style type="text/css">
		#infoch_label, #mid_label, #mnick_label, #mpwd_label, #rempwd_label, #mtel1_label, #memail_label, #maddr1_label {
			color : red;
		}
		</style>
		...
		<table class="table table-hover">
			<tbody>
				...
				<tr>
					<th> 업 체 명 </th>
					<td>
						<input type="text" id="mnick" name="mnick" maxlength="20" class="form-control">
						<label for="mnick" id="mnick_label"></label>
						<p class="tip">
						한글, 영문소문자, 영문대문자, 숫자만 입력해주세요.
						</p>
					</td>
				</tr>
				...
				<tr>
					...
					<td>
						...
						<div class="input-group">
							<span class="input-group-text"> 업 체 위 치 </span>
							<input type="text" id="maddr1" name="maddr1" readonly="readonly"
									class="form-control">
						</div>
						<div class="input-group">
							<span class="input-group-text"> 업 체 상 세 주 소 </span>
					...
					<td>
						<div class="input-group mb-1">
							<div class="input-group-text">
								<input type="checkbox" id="infoch" name="infoch" class="mr-1"
...
• 경찰청 사이버테러대응센터 (www.ctrc.go.kr / 182)</textarea>
						<label for="infoch" id="infoch_label"></label>
					</td>
				</tr>
				<tr>
					<th> S M S 수 신 동 의 </th>
		...
		<div class="clearfix">
			<button id="join_btn" class="btn btn-primary float-right"> 신 청 </button>
		</div>
		<hr>
	...
	let onlyPwd = /^[a-z0-9~!@#$%^&*().]+$/;
	let checkedEM = "";
	let onlyNN = /^[a-zA-z가-힣0-9]+$/;
	let onlyEM = /^[a-zA-Z0-9@.]+$/;

	$(document).ready(function() {
		$("#join_btn").click(function() {

			if( $("#infoch").prop("checked") == false ){
				$("#infoch_label").text("필수 선택 사항 입니다.");
				return;
			} else { $("#infoch_label").text(""); }
			...
			if( $.trim( $("#mnick").val() ) == "" || $.trim( $("#mnick").val() ).match(onlyNN) == null ){
				$("#mnick_label").text("업체명입력을 확인 후 다시 해주세요.");
				return;
			} else { $("#mnick_label").text(""); }
			...
			if( $("#maddr1").val() == ""){
				$("#maddr1_label").text("업체위치입력을 해주세요.");
				return;
			} else { $("#maddr1_label").text(""); }
			
			if( $.trim( $("#maddr2").val() ) == ""){
				$("#maddr1_label").text("업체상세위치입력을 확인후 다시 해주세요.");
				return;
			...
			$.post(
					...
					, {
						...
						, emailch : $("#emailch").prop("checked")
						, smsch : $("#smsch").prop("checked")
					}
					, function(data, status) {
						if(data == 1){
							alert("판매자 등록 신청에 성공 하셨습니다.");
							alert("관리자 승인 후 이용 가능하니, 잠시 기다려주세요.");
